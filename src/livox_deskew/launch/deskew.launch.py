...
def generate_launch_description():
    ...
    max_missing_ratio_arg = DeclareLaunchArgument(
        'max_missing_ratio',
        default_value='0.02',
        description='Maximum ratio of points that can fail interpolation before dropping scan'
    )
    ...
    use_odom_fallback_arg = DeclareLaunchArgument(
        'use_odom_fallback',
        default_value='true',
        description='Use odometry topic as fallback when TF is unavailable'
    )

    # Create the deskew node
    deskew_node = Node(
        package='livox_deskew',
        executable='livox_deskew_node',
        name='livox_deskew_node',
        output='screen',
        parameters=[{
